...
async def rebuild_cache(
    ...
    """Rebuild the cache from the database, optionally save to disk and update in-memory.
    ...
    3. Optionally saves to disk for other processes to load
    4. Optionally updates the in-memory app_cache with atomic swap
    ...
    Args:
        ...
        save_to_disk: If True, save cache to disk after building.
            Set to False for web app startup (only worker should save).
    ...
    try:
        ...
        logger.debug(
            f"Found {len(active_roles)} active roles out of {len(all_role_snapshots)} total"
        )
        ...
        # Collect RoleDefinition objects directly (no dict conversion needed)
        ...
        # Only worker should save - web app just loads from disk.
        if save_to_disk:
        ...
        if update_in_memory:
            ...
            action = (
                "swapped in-memory and saved to disk" if save_to_disk else "swapped in-memory only"
            )
            logger.debug(f"Precomputed caches, {action}")
        ...
        try:
            ...
            # Pass RoleDefinition objects from active roles
            role_definitions = [
                role.role_definition for role in active_roles if role.role_definition
            ]
            ai_recommender.initialize(role_definitions)
        ...
        logger.exception("Failed to rebuild cache: %s", e)
        return False
    finally:
